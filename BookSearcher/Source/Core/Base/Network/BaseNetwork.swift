...
protocol BaseNetworkProtocol {
    ...
    func requestJson(with target: Target, completion: @escaping (_ json: [String: Any]?, _ error: BaseNetworkError?) -> Void)
    func request(with target: Target, completion: @escaping (_ data: Data?, _ error: BaseNetworkError?) -> Void)
}
    ...
    func requestJson(with target: Target, completion: @escaping (_ json: [String: Any]?, _ error: BaseNetworkError?) -> Void) {
        ...
        request(with: url, method: target.method) { data, error in
            ...
            do {
                ...
                completion(nil, .wrongResponseError)
            }
        }
    }
...
private extension BaseNetwork {
    func request(with url: URL, method: HttpMethod, completion: @escaping (_ data: Data?, _ error: BaseNetworkError?) -> Void) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            session.dataTask(with: request) { [weak self] data, response, error in
                DispatchQueue.main.async { [weak self] in
                    ...
                    guard let data = data else {
                        completion(nil, .serverSideError(code: httpResponse.statusCode))
                        return
                    }
                    completion(data, nil)
                }
